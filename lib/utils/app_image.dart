class AppImage {
  AppImage._();

  static const String _svgPath = "assets/svg/";
  static const String _pngPath = "assets/image/";
  static const String _lottiePath = "assets/lottie/";

  static const String icBack = "${_svgPath}ic_back.svg";
  static const String icDone = "${_svgPath}ic_done.svg";
  static const String icCircleClose = "${_svgPath}ic_circle_close.svg";
  static const String icWarning = "${_svgPath}ic_warning.svg";
  static const String icClock = "${_svgPath}ic_clock.svg";
  static const String icTrash = "${_svgPath}ic_trash.svg";
  static const String icArrowDown = "${_svgPath}ic_arrow_down.svg";
  static const String icFemale = "${_svgPath}ic_female.svg";
  static const String icMale = "${_svgPath}ic_male.svg";
  static const String icMaleFemale = "${_svgPath}ic_male_female.svg";

  static const String imgHeartRate = "${_pngPath}img_heart_rate.png";
  static const String imgBloodPressure = "${_pngPath}img_blood_pressure.png";
  static const String imgWeightBmi = "${_pngPath}img_weight_bmi.png";
  static const String imgAdd = "${_pngPath}img_add.png";
  static const String imgHeartRateTutorial = "${_pngPath}img_heart_rate_tutorial.png";
  static const String imgSplash = "${_pngPath}img_splash.png";

  static const String lottieWeightScale = "${_lottiePath}weight_scale";
}
